...
mfcc_config_dict = {
    'n_mels': 20,  # Number of mel filters
    'ref_power': 'max',  # Reference function to use for the logarithm power.
    'fmax': 8000
    }
...
sampling_feat_config_dict = {'W': 16,
                            }
sampling_config = SubConfig(sampling_feat_config_dict)
config.add_subconfig('sampling', sampling_config)
...
config.double_beats = 1
config.n_conditions = 2
config.cutoff_len = 1800
config.n_embedding = 64
config.hop_length = 256
config.n_training_samples = 1e6
config.n_val_samples = 1e6
